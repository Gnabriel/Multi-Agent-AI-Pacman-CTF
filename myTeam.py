...
class DummyAgent(CaptureAgent):
    ...
    def is_target(self, gameState: GameState, pos: Tuple[int, int]):
        if self.is_pacman:
            ...
            if not self.is_close_to_ghost(
                gameState=gameState, pos=self.get_current_pos(gameState=gameState), reversed=True,
            ):
    ...
    def is_close_to_ghost(self, gameState: GameState, pos: Tuple[int, int], reversed: bool = False):
        return any(
            [
                sqrt(
    ...
    def chooseAction(self, gameState: GameState):
        ...
        if gameState.data.score < 0 and len(self.positions) > 50 and len(set(self.positions[-50:])) < 2:
